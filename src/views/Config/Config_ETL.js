...
export default function ConfigETL() {
  ...
  const setDatos = async () => {
    await getETLSchedule().then((res) => {
      // console.log(res.ScheduleJob.Occurs_detail);
      Setnextrundate(
        "Ocurre " +
          res.ScheduleJob.Occurs_detail +
          " " +
          res.ScheduleJob.Frequency
      );
      ...
      Setactive_start_date(
        moment(res.ScheduleJob.active_start_date.toString()).format(
          "YYYY-MM-DD"
        )
      );
      Setactive_start_time(
        moment(
          res.ScheduleJob.active_start_date +
            " " +
            res.ScheduleJob.active_start_time
        ).format("HH:mm:ss")
      );
      Setactive_end_date(
        moment(res.ScheduleJob.active_end_date.toString()).format("YYYY-MM-DD")
      );
      Setactive_end_time(
        moment(
          res.ScheduleJob.active_end_date +
            " " +
            res.ScheduleJob.active_end_time
        ).format("HH:mm:ss")
      );
      return res;
    });
    ...
    <Card>
      <CardHeader color="primary">
        <h4 className={classes.cardTitleWhite}>
          Gestión de la programación del proceso ETL
        </h4>
        <p className={classes.cardCategoryWhite}>{nextrundate}</p>
      </CardHeader>
      <form
        ...
        <CardBody>
          <GridContainer>
            ...
            <GridItem xs={12} sm={12} md={6}>
              ...
              >
                <InputLabel id="select-freqtype-historic">
