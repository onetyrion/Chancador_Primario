...
export default function ProfileSecurity(props) {
  ...
  const changePassword = async (e) => {
    if (e.target.pass_current.value.length === 0) {
      notify.show(
        "Debe Ingresar la contraseña Actual, y las nuevas contraseña deben coincidir",
        "error"
      );
    } else if (
      e.target.newPassword.value !== e.target.confirmNewPasword.value
    ) {
      notify.show(
        "Debe Ingresar la contraseña Actual, y las nuevas contraseña deben coincidir",
        "error"
      ...
      if (!e.target.newPassword.value && !e.target.confirmNewPasword.value) {
        ...
        await PutLoginAPI(
          e.target.confirmNewPasword.value,
          e.target.pass_current.value
        ).then((value) => {
          if (value.error) {
  ...
  const desactiveAccount = async (event) => {
    if (event) {
      await PutUsersAPI({ pass: event, Estado: 0, desactiveUser: true }).then(
        (value) => {
          console.log(value);
          ...
          <Card>
            <CardHeader color="warning">
              ...
              <p className={classes.cardCategoryWhite}>
                Es una buena idea utilizar una contraseña segura que no esté
                utilizando en otro lugar.
              </p>
            </CardHeader>
            <CardBody>
              <form
        ...
        <GridItem xs={12} sm={12} md={12}>
          <Card>
            <CardHeader color="danger">
              ...
              <p className={classes.cardCategoryWhite}>
                Recuerda que no podrás volver a activarla, solo el administrador
                del sistema puede hacerlo.
              </p>
            </CardHeader>
            <CardBody>
              <GridContainer>
                <form
                  className={classes.formDesactive}
                  onSubmit={(a) => {
                    desactiveAccount(a.target.off_pass_current.value);
                    a.preventDefault();
                  }}
                >
                  {/* //Actual Contraseña */}
